...
\begin{method}{Cauchy Produkt}
	Falls $\sum_{n} a_n$ und $\sum_n b_n$ absolut konvergierten, konvergiert ihre Cauchy Produkt und es gilt:
...
\begin{method}{Potenzreihe / Konvergenzradius}
	...
	$\rho = \limm \left|  \frac{a_n}{a_{n+1}}\right|$ oder eben $\rho = \frac{1}{\limm \sqrt[n]{|a_n|}}$.
\end{method}
...
Eine Funktion $f : D \to \mathbb{R}$ nennt man stetig in $x_0$ falls 
$$\forall \epsilon > 0 \ \exists \delta > 0 \ : \ |x-x_0| < \delta \implies |f(x) - f(x_0)| < \epsilon$$ beziehungsweise auf $\mathbb{R}$.

$$f((x_0 - \delta , x_0 + \delta)) \subset (f(x_0) - \epsilon, f(x_0) + \epsilon)$$
Äquivalent dazu gibt es das Folgenkriterium: Für jede Folge $x_n \in \Omega$ mit $\limm x_n = a$ gilt $\limm f(x_n) = f(a)$. 
\end{method}
